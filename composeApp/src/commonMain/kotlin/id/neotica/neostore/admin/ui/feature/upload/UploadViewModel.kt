...
class UploadViewModel(
    ...
    private fun updateQueueStatus(file: PlatformFile, status: FileStatus, error: String? = null) {
        _uiState.update { state ->
            val newQueue = state.uploadQueue.map {
                if (it.file.path == file.path) it.copy(status = status, errorMessage = error) else it
            }
            state.copy(uploadQueue = newQueue)
        }
    }

    fun addFilesToQueue(files: List<PlatformFile>) {
        ...
        if (_uiState.value.uploadQueue.size == 1 && !uiState.value.isBulkProcessing) {
            setPath(_uiState.value.uploadQueue.first().file)
        }
    }
    ...
    private suspend fun analyzeApkSynchronously(file: PlatformFile): Boolean = withContext(Dispatchers.IO) {
        try {
            currentFile = file
            _uiState.update { it.copy(filePath = file.path, statusMessage = "Analyzing ${file.name}...", uploadProgress = 0f) }
    ...
    private suspend fun uploadSynchronously(): String? {
        ...
        val file = currentFile ?: return "No file selected"
    ...
    fun setPath(file: PlatformFile) {
        currentFile = file
        _uiState.update {
            it.copy(filePath = file.path, statusMessage = "Analyzing APK...", uploadProgress = 0f)
        }
